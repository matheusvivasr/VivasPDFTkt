ordem = {
    "meia": [1, 4, 2, 3],
    "quarto": [1, 8, 4, 5, 7, 2, 6, 3],
    "oitavo": [1, 16, 8, 9, 15, 2, 10, 7, 3, 14, 6, 11, 13, 4, 12, 5],
}
...
def ordenador(entrada, modo):
  ...
  for i in range(P):
    ...
    d = i % D
    if not d: step += 1
    pagina = (seq[d] + step * D) - 1

    # Caso tenham paginas que sobram
    if pagina >= L: reordered.append(0)
    else:
      # Paginas a rodar
      if (d % 4 == 0) or (d % 4 == 1): entrada[pagina].rotate(180)
        
      # Enfim, adiciona a pagina
      reordered.append(entrada[pagina])
